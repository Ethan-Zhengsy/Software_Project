...
3. **分支管理**
   ...
   - 本项目采用以下分支管理策略：
     ...
     - `dev`分支：开发分支，团队成员的日常开发工作在此分支进行，包含docs文件夹用于存放文档类文件(做类图和需求文档等不需要启用)
...
4. **工作流程建议**
   ...
   - 开始新工作前先到对应分支dev或者main拉取最新代码 `git pull origin dev/main（根据分支选择）`
   - 根据任务类型在dev创建对应分支
   - 在分支上进行开发并频繁提交
   - 完成工作后推送到远程并创建Pull Request
   - 团队成员审核代码后合并

   > 注意：在做文档类的任务直接上传到main分支对应目录，dev分支等编写代码启用
   >
5. **实际操作示例**
